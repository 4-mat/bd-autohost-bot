...
var clearance = ['mafiassatan','lifehaxgamer']; //if (clearance.indexOf(user.id) == -1) return false;
var accountName = 'lifehaxgamer'; //NOTE: currently really dirty way of doing it atm
...
exports.commands = {
	...
	// this is a bot for when you get hosted
	bp: "host",
	host: function(arg, user, room){
		if (toID(arg) !== accountName) return false;
		this.say(room, "%gchat");
		this.say(room, '/join groupchat-battledome-one');
		this.say(room, '/join groupchat-battledome-two');
		this.say(room, '/join groupchat-battledome-three');
		this.say(room, '/join groupchat-battledome-four');
		this.say(room, '/join groupchat-battledome-five');
		this.say(room, '/join groupchat-battledome-six');
		this.say(room, '/pm ' + partnerName + ', %ghettosearch');
		turnCount = 0;
		currentMove = '';
	},
	ghettosearch: function(arg, user, room){
		this.say('/pm icekyubs, %sco skirm,wand');
		this.say(room, '/join groupchat-battledome-one');
		this.say(room, '/join groupchat-battledome-two');
		this.say(room, '/join groupchat-battledome-three');
		let squadList = ['one', 'two', 'three', 'four', 'five', 'six'];
		this.say(room, '/pm ' + partnerName + ', squadList loaded');
		console.log('is this even working lol');
		for (let i = 0; i < squadList.length; i++) {
			const pass = squadList[i];
			this.say(room, '/pm ' + partnerName + ', checking ' + pass + ' squad');
			//console.log(pass);
			fastCheck = user.hasRank('groupchat-battledome-' + pass, '%');
			if (fastCheck){
				console.log(pass + ' squad is where he is driver');
				this.say('/pm ' + partnerName + ', found you in squad ' + pass);
				this.say('/pm ' + partnerName +", %gc " + pass);
				group('squadpass').set('array', pass);
				SQUAD = group('rollpass').get('array');
				return this.say('/pm ' + partnerName + ', %start')
			}
		}
	},
	start: function(arg, user, room){
		this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', %addp ' + partnerName);
		this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', %setgame 1vE');
		this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', %info');
		this.say(room, '/pm ' + partnerName + ', %cookingtime');
	},
	cookingtime: function(arg, user, room){
		this.say(room, '/pm ' + partnerName + ', %itsready')
	},
	itsready: function(arg, user, room){
	let htmlstuff = group('htmlpass').get('array');
		if(!htmlstuff.includes('Devil')) {
			this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', %info');
			htmlstuff = group('htmlpass').get('array');
		}
		htmlstuff = group('htmlpass').get('array');
		htmlstuff = htmlstuff.toString();
		let turnorder = htmlstuff.split('Turn Order:');
		turnorder = turnorder[1].split('<');
		turnorder = turnorder[0];
		this.say(room,'/pm ' + partnerName + ', ' + turnorder);
		turnorder = turnorder.split(',');
		switch (toID(turnorder[0])) { // turn 1
			case 'devilsdragon':
				return this.say(room, '/pm ' + partnerName + ', %returnnext');
			break;
			case partnerName:
				this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', %cut 1');
				this.say(room, '/pm ' + partnerName + ', %m n,4')
				this.say(room, '/pm ' + partnerName + ', %idle');
			break;
		}
	},
	returnnext: function(arg, user, room){
		return this.say(room, '/pm ' + partnerName + ',%next')
	},
	next: function(arg, user, room){
		this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', %info')
		turnCount = turnCount + 1;
		switch (turnCount) {
			case 1:
				this.say(room, '/msgroom groupchat-battledome-' + SQUAD + ', /me cavein @ ' + partnerName);
				currentMove = 'cavein';
				this.say(room, "%r 1d20");
				break;
			case value:
	...
	testr: function(arg, user, room){
		let rollstuff = group('rollpass').get('array');
		rollstuff = rollstuff.toString();
		return this.say(room, '/pm ' + user.id + ', !code ' + (rollstuff !== null ? rollstuff : "No roll things saved"));
	},
	...
	gc: function (arg, user, room) { // should be used to get the partner acc into the gchat
		if (room !== user && clearance.indexOf(user.id) == -1) return false;
		SQUAD = arg;
		group('squadpass').set('array', SQUAD);
	...
	timer: 'starttimer',
 	starttimer: function (arg, user, room) {
		//if (room === user || room.id !== "groupchat-lifehaxgamer-test") return false;
		...
		this.buzzer = setTimeout(function (room, buzzMessage) {
			...
			this.buzzed = '';
		}.bind(this), Number(arg) * 1000, room, 'Time is up, ' + user.name + '!');
	},
	stop: 'stoptimer',
	stoptimer: function (arg, user, room) {
	 	if (isNaN(Number(arg))) return false;
		//if (!this.buzzed || room === user || room.id !== "groupchat-lifehaxgamer-test") return false;
		//var hostarray = group('host').get('array');
		//if (!user.hasRank(room.id, '+') && hostarray.indexOf(user.id) == -1) return false;
	...
	htmlthing: function(arg, user, room){
		this.uncacheTree('./commands.js');
		Commands = require('./commands.js').commands;
		let htmlstuff = group('htmlpass').get('array');
		htmlstuff = htmlstuff.toString();
		this.say(room, '/pm ' + user.id + ', !code ' + (htmlstuff !== null ? htmlstuff : "No html things saved"));
		htmlstuff = "";
	}*/
	js: function (arg, user, room) {
		//if (!user.isExcepted()) return false;
	...
	start: function(arg, user, room){
		return this.say(room,
			"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
			'<button name="send" value="/pm icekyubs, %bp lifehaxgamer &#10;/pm lifehaxgamer, Rolsetgc"> %bp lifehaxgamer </button>' +
			""
		)
	},
	setgame: function(arg, user, room){
		roomID = arg;
		return this.say(room, 
			"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
			'<button name="send" value="/msgroom bd, %gchat ' + // should be gchat
			'&#10;/join ' + roomID +
			'&#10;/pm thatcableguy, ^button pregame2' +
			'">Pregame</button>'
		)
	},
	button: function(arg, user, room){
		switch (arg) {
			case "pregame2":
			return this.say(room, 
				"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
				'<button name="send" value="' +
				'/msgroom ' + roomID + ', %addp lifehaxgamer' +
				'&#10;/msgroom ' + roomID + ', %setgame 1vE' +
				'&#10;/msgroom ' + roomID + ', %inf' +
				'">Pregame2</button>'
				)
				break;
			...
			return this.say(room,
				"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
				'<button name="send" value="' +
				'/msgroom ' + roomID + ', /pm lifehaxgamer, Rollastmove ci' +
				'&#10;/msgroom ' + roomID + ', /pm lifehaxgamer, Rolmem ci' +
				'&#10;/pm thatcableguy, ^button turn1part3' +
				'">turn1part2</button>'
				)
				break;
			case "turn1part3":
			return this.say(room,
				"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
				'<button name="send" value="' +
				'/msgroom ' + roomID + ', /me cavein @ lifehaxgamer' + 
				'&#10;/msgroom ' + roomID + ', %r 20' +
				'">turn1part3</button>'
				)
				break;
			...
			return this.say(room, 
				"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
				'<button name="send" value="' +
				'/msgroom ' + roomID + ', /pm lifehaxgamer, Roltunnel' + 
				'&#10;/msgroom ' + roomID + ', /pm lifehaxgamer, Rolm i,4' +
				'&#10;/pm thatcableguy, ^button cidmg3' + 
				'">cidmg2</button>'
				)
				break;
			case "cidmg3":
			return this.say(room,
				"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
				'<button name="send" value="' +
				'/msgroom ' + roomID + ', /pm lifehaxgamer, Rolmem bl' +
				'&#10;/msgroom ' + roomID + ', /pm lifehaxgamer, Rollastmove bl' +
				'&#10;/msgroom ' + roomID + ', /pm lifehaxgamer, Rolbl ener hwis' +	
				'">cidmg3</button>'
			)
			case "bldmg2":
			return this.say(room,
				...
				'<button name="send" value="' +
				'/msgroom ' + roomID + ', /pm lifehaxgamer, Rolmem bl' +
				'&#10;/msgroom ' + roomID + ', /pm lifehaxgamer, Rollastmove fb' +
				'&#10;/msgroom ' + roomID + ', %r 3d20 // fb @ lifehaxgamer' +
				'">bldmg2</button>'
			)
			case "mm":
			default:
	...
	setsquadto: function(arg, user, room){
		let args = arg.split(',');
		if (toID(args[0]) !== 'devilsdragon1' && turnCount === 0 ) this.say(room, '/pm lifehaxgamer, Rolc /msgroom ' + roomID + ', %move n,4 // idle');
		return this.say(room, 
			"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
			'<button name="send" value="/msgroom ' + roomID + ', %info ' +
			'&#10;/msgroom ' + roomID + ', %movem h,4' +
			'&#10;/pm thatcableguy, ^button turn1part2' +
			'">Turn1</button>'
		)
	},
	rroll: function(arg, user, room){
		this.say(room,"oh cool i got a roll");
		let args = arg.split('w ');
		let damage; let mr; let acc; let damageRoll;
		switch (args[1]) {
			case "cidmg": // 7,6 || Total: 18 w dmg
				damage = args[0].slice(args[0].indexOf(':') + 2,args[0].length);
				return this.say(room, 
					"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
					'<button name="send" value="/msgroom ' + roomID + ', %hp -' + damage + ', lifehaxgamer' +
					'&#10;/msgroom ' + roomID + ', %info' + 
					'&#10;/msgroom ' + roomID + ', /pm thatcableguy, ^button cidmg2' + 
					'">CIDamage</button>'
					);
				break;
			case "ci":
				...
				acc = parseInt(args[0]);
				if (acc < mr){
					return this.say(room, "/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
					'<button name="send" value="/msgroom ' + roomID + ', %info '+
					'&#10;/msgroom ' + roomID + ', /pm thatcableguy, ^button cidmg2' +
					'">CI missed</button>'
					)
				...
				damageRoll = "1d8+11";
				if (acc === 20){damageRoll = "2d8+11"}
				return this.say(room, 
					"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
					'<button name="send" value="/msgroom ' + roomID + ', /pm lifehaxgamer, Rolmem cidmg' + 
					'&#10;/msgroom ' + roomID + ', %tile stone,m,3,m,4' +
					'&#10;/msgroom ' + roomID + ', %r ' + damageRoll +
					'">CI hit</button>'
				)
				break;
			case "bldmg":
				return this.say(room, 
					...
					'<button name="send" value="/msgroom ' + roomID + ', %hp -' + damage + ', lifehaxgamer' +
					'&#10;/msgroom ' + roomID + ', %info' + 
					'&#10;/msgroom ' + roomID + ', /pm thatcableguy, ^button bldmg2' + 
					'">CIDamage</button>'
				);
				break;
			case "bl":
				...
				if (acc < mr){
					...
					'<button name="send" value="/msgroom ' + roomID + ', %info '+
					'&#10;/msgroom ' + roomID + ', /pm thatcableguy, ^button bldmg2' +
					'">blight missed</button>'
					)
				}
				damageRoll = "5d6+28";
				if (acc === 20){damageRoll = "8d6+28"}
				return this.say(room, 
					"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
					'<button name="send" value="/msgroom ' + roomID + ', /pm lifehaxgamer, Rolmem bldmg' + 
					'&#10;/msgroom ' + roomID + ', %hp -16' +
					'&#10;/msgroom ' + roomID + ', %r ' + damageRoll + '// psn included' +
					'">blight hit</button>'
				)
			case "fbdmg":
				break;
			case "fb":
				//^rroll 18,1,5 || Total: 24 
				let pass = args[0].slice(0,args[0].indexOf(' '));
				let accArray = pass.split(",");
				var accBonus = 0; var hitNumber = 0; var critNumber = 0; var critThreshold = 18;
				for (let i = 0; i < accArray.length; i++) {
					const element = accArray[i];
					let mr = 5 + accBonus;
					if (element < mr){
						this.say(room, accBonus);
						continue;
					}
					hitNumber = hitNumber + 1;
					if (element >= critThreshold){
						critNumber = critNumber + 1;
						critThreshold = critThreshold - 1;
					}
				}
				let X = hitNumber + critNumber; let Y = 6; let Z = hitNumber + 2 + 6;
				let dmgRoll = `${X}d${Y}+${Z}`
				if (hitNumber === 0) {
					return this.say(room, "/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
						'<button name="send" value="' +
						'/msgroom ' + roomID + ', %info' + 
						'&#10;/msgroom ' + roomID + ', /pm thatcableguy ^button mm' + 
						'">fbmissed</button>'
					)
				};
				this.say(room, 'i think the damage roll is ' + dmgRoll + '// burn included');
				return this.say(room, 
					"/msgroom botdev, /sendhtmlpage " + partnerName + ", devilsdragonhost, " +
					'<button name="send" value="' +
					'/msgroom ' + roomID + ', %r ' + dmgRoll + '// burn included' +
					'&#10;/msgroom ' + roomID + ', %info' + 
					'&#10;/msgroom ' + roomID + ', /pm thatcableguy ^button mm' + 
					'">fbstuff</button>'
					)
