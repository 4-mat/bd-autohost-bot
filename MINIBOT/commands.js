...
var SQUAD;
var clearance = ['mafiassatan','mymemoryisbad','boicamer','somethin9112','outm']; //if (clearance.indexOf(user.id) == -1) return false;
...
exports.commands = {
	...
	cut: function (arg, user, room) {
		if (room !== user /*&& clearance.indexOf(user.id) == -1*/) return false;
		let text = '/msgroom battledome, %cut '+ (arg ? arg : "");
		return this.say(room, text);
	},
	...
	mm: function(arg, user, room){ // mana missiles
		...
		for (let i = 0; i < args.length; i++) {
			...
			if (element === "ener" || element === "outman" || element === "hwis"){
				text = text + element;
			};
		}
	...
	bl: function(arg, user, room){ // blight
		...
		this.say(room, text);
		this.say(room, '/pm mafiassatan, queue max sac ener manamissiles @ Mafia\'s Satan');
		return this.say(room, '/msgroom groupchat-battledome-'+SQUAD+',%r 20');
	},
	...
	htmlthing: function(arg, user, room){
		var htmlstuff = group('htmlpass').get('array');
		htmlstuff = htmlstuff.toString();
		console.log(htmlthing);
		return this.say(room, '/pm ' + user.id + ', !code ' + htmlstuff);
	}
};
