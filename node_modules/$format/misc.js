...
// write a function for toID

function toID(str) {
    console.log('in: ' + str);
    let returnstatement = str.toLowerCase().replace(/[^a-z0-9]/g, "");
    console.log('out: ' + returnstatement);
...
module.exports =  {
    ...
    uhtmlnotif: function(data, room){
        ...
        if (data.includes('playerdata')){
            uhtml('uhtmldata').get('lastUHTMLmessage');
            
            uhtml('uhtmldata').set('votingStage', data);
        }
        if (data.includes('joinleave')){
            let ugaming = html('uhtmldata').get('lastUHTMLmessage');

            let returnStatement = `${htmlpageIntro}, ${ugaming}`;
            return returnStatement + '<br>' + data + '&#10;/pm 4mat, this is just a join message lol';
        }
    ...
    makeButtonsForWeapon: function(weapon, level, room = 'battledome', stat = {"A": "10", "M": "10"}){
        ...
        // if the weapon's name in the weapon variable is Cards change it to Tarot Cards
        ...
        if(weaponIndex.indexOf(weapon) > -1){
            ...
            for (let i = 0; i < branchIndex.length; i++){
                //console.log('go find this shit ' + data('branch').get(branchIndex[i]));
                let stuffThatNeedsIteration = (data('branch').get(branchIndex[i]))
                //stuffThatNeedsIteration = JSON.stringify(stuffThatNeedsIteration);
                console.log(stuffThatNeedsIteration);
                // make a foreach loop that goes through the stuffThatNeedsIteration and uses toID on it using forEach 
                const ids = [];
                [...stuffThatNeedsIteration].forEach(item => {
                  let id = toID(item);
                  console.log('this is probably the issue:' + id);
                  ids.push(id);
                });
                ...
                if (concatenatedIds.includes(toID(weapon))){
                    // get the weapon's abilities
                    //console.log('gaming: ' + JSON.stringify(data(branchIndex[i]).get(toID(weapon))));
                    console.log('gaming: ' + JSON.stringify(data(branchIndex[i]).keys()));
                    console.log('weapon: ' + weapon);
                    // time to rewrite this :)
                    console.log(data('branch'));

                    const ids2 = [];
                    [...data(branchIndex[i]).keys()].forEach(item => {
                        const id2 = toID(item);
                        ids2.push(id2);
                    });

                    const concatenatedIds2 = ids2.join(','); 

                    let wedoithere = 0;
                    //if (concatenatedIds2.includes(toID(weapon))){
                        // check what index concatenatedIDs2 includes toID(weapon)
                        for (let k = 0; k < concatenatedIds2.length; k++) {
                            console.log(concatenatedIds2[k]);
                            if (concatenatedIds2[k] === toID(weapon)){
                                // the weapon id is found at index i
                                wedoithere = k;
                                break;
                            }
                        }
                    ...
                    let heyyyyy = data(branchIndex[i]).keys();
                    abilities = data(toID(branchIndex[i])).get(heyyyyy[wedoithere]);
                    console.log(abilities);
                    // remove the abilities greater than the user's current level
                    abilities = abilities.filter(ability => Number(ability.Lvl) <= level);
                    console.log(abilities);
                    break;
                }
            }
        ...
        for (let i = 0; i < abilities.length; i++){
            ...
            let MR = "MR " + ability.MR;

            if(ability.MR === "-"){
                MR = "";
            }
            let button2 = `<button class='button' name='send' value='/pm thatcableguy, %use ${ability.Name}'>${ability.Name}</button>`;
            let button  = `<button class='button' name='send' value='/msgroom ${room}, ${ability.Name} @ target ${roll} ${MR}'>target</button>`;
        ...
        let uhtmlthing = require('origindb')('uhtml'); 
        uhtmlthing('uhtmldata').set('weaponmoves', returnStatement);
        console.log('hey this is returnstatement: ' + JSON.stringify(returnStatement));
        return returnStatement2;
    },
    makeButtonsForClass: function(_class, level, room = 'battledome', stat = {"A": "0", "M": "0"}){
        let data = require('origindb')('data');

        console.log(_class); // Rifter

        if (_class === "CryoKinetic" || _class === "cryokinetic"){ 
            _class = "Cryokinetic";
        }

        if (_class === "PyroKinetic" || _class === "pyrokinetic"){ 
            _class = "Pyrokinetic";
        }

        console.log(_class); // Pyrokinetic

        // get the class abilities
        let abilities = data('classes').get(_class);
        abilities = data('classes').get(_class);
        ...
        for (let i = 0; i < abilities.length; i++){
            ...
            let button2 = `<button class='button' name='send' value='/pm thatcableguy, %use ${abilityName}'>${abilityName}</button>`;
            let button  = `<button class='button' name='send' value='/msgroom ${room}, ${abilityName} @ target ${roll} ${MR} &#10;/pm thatcableguy, %cut recent'>target</button>`;
    ...
    getStats: function(playerName = "all"){
        ...
        for (let i = 0; i < PLstats[0].length; i++){
            if (PLstats[0][i]["Name"]){
                if (toID(PLstats[0][i]["Name"]) === toID(playerName)) {
                    return PLstats[0][i];
                }
            }
            return undefined;
    ...
    saveCutNumber(number){
        let uhtml = require('origindb')('uhtml');
        return uhtml('uhtmldata').set('cutNumber', number);
    },
    getCutNumber(){
        let uhtml = require('origindb')('uhtml');
        return uhtml('uhtmldata').get('cutNumber');
    },
    getLastInfo(){
        let uhtml = require('origindb')('uhtml');
        return uhtml('uhtmldata').get('lastINFO');
    }
}