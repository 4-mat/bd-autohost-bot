...
module.exports =  {
            ...
            for (let i = 0; i < myArray.length; i++) {
                ...
                if (element.includes('</b')){
                    ...
                    if (pass[0].includes("P")){
                        ...
                        let entry = pass[0] + " Position: " + String.fromCharCode(65 + rowCount) + ", " + elementCount + " " + Pcount;
                        if (elementCount === 0){
    ...
    makeButtonsForWeapon: function(weapon, level){
        let branch = require('data')('branch');
        // check if the weapon is in the branch.weapon
        if (branch.weapon.includes(weapon)){
            //we find what branch the weapon is in
            let branchWeapon = branch.weapon.filter(e => e === weapon);
            //now check the branch file for the weapon 
            let branchFile = require('data')('branch').filter(e => e.weapon === branchWeapon)  
            //now make the buttons
            let buttons = '';
            for (let i = 0; i < branchFile.length; i++) {
                buttons += `<button class="btn btn-primary" onclick="uhtml('uhtmldata').set('playerPosition', '${branchFile[i].position}')">${branchFile[i].name}</button>`;
            }
            return buttons; 
        }
        else{
            return 'THIS WEAPON DOES NOT EXIST';
        }
    },
