...
module.exports =  {
    ...
    uhtmlnotif: function(data, room){
        ...
        if(data.includes('<summary>Map</summary>')){
            ...
            let pass; let mapText = []; let columnCount = 0; let elementCount = 0;
    ...
    makeButtonsForWeapon: function(weapon, target){
        if (weapon === "cards"){
            button1 = '<button name="send" value="/msgroom groupchat-battledome-one, /me discard @ ' + target + '">Discard</button>'
            return button1;
        }
    },
    getClassAndWeap: function(target){
        ...
        json.forEach(element => {
            for (let i = 0; i < element.length; i++) {
                console.log('hey listen ' + element);
                if (toID(element.Name) === toID(target)){
                    return playerList[0][i]['Class/Weapon'];
                }
            }
        });
    }
